...
sweep_dir = Path("../persist/cw_am_sweep")
fig_dir = Path("../figures/am_sweep")
...
for path in files:
    print(f"=== {path.name} ===")
    try:
        counts, freqs, keys = load_data(path)   # shape (n_sweep, n_repeats, 2)
        ...
        if freqs is None:
            ...
            if keys is not None:                # was an npz but no usable axis
                print(f"  no usable freq axis (keys: {keys}); "
                      f"assuming {n_sweep} pts over "
                      f"{start_freq/1e9}-{stop_freq/1e9} GHz")

        detuning = freqs - center_freq

        # AM contrast: MW-off minus MW-on, normalised by MW-off, averaged over repeats
        active = counts[:, :, 0]
        inactive = counts[:, :, 1]
        am_counts = np.mean((inactive - active) / inactive, axis=1)
        ...
        peak_MHz = max_peak_detuning / 1e6
        slope_MHz = max_slope_detuning / 1e6
        print(f"  Max peak: {peak_MHz:.3f} MHz "
              f"({(center_freq + max_peak_detuning)/1e9:.6f} GHz)")
        ...
        ax.plot(detuning / 1e6, am_counts, marker="o", ls="none", ms=3.2,
                mfc="none", mec="0.35", mew=0.8, label="Data")
        ax.plot(detuning / 1e6, fit, color="C3", lw=1.4, label="Lorentzian fit")
        ax.axvline(peak_MHz, color="C0", ls="--", lw=1.0,
                   label=rf"Max peak ($\delta = {peak_MHz:.2f}$ MHz)")
        ax.axvline(slope_MHz, color="C1", ls=":", lw=1.0,
                   label=rf"Max slope ($\delta = {slope_MHz:.2f}$ MHz)")

        ax.set_xlabel(r"Detuning $\delta = f - D$ (MHz)")
        ax.set_ylabel(r"AM contrast")
